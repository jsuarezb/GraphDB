...
public class CallMemberGenerator {
  ...
  private static final int EMISORRECEPTOR_BASE_COUNT = 300;
  private static final int EMISORRECEPTOR_COUNT = 300;

  private static final String PATH = "../lib/";

  private static final String CREATE_SCHEMA_SQL = PATH + "dbschema-create";
  private static final String EMISOR_RECEPTOR_SQL = PATH + "emisorreceptor_final";

  private static final int START_ID = 4;

  public static void main(String[] args) throws IOException, ClassNotFoundException, SQLException {
    List<Integer> amounts = Arrays.asList(200, 300, 500);
    int i = START_ID;
  ...
  private static void createFile(int schema, int callAmount, int userAmount) throws IOException {
    ...
    auxStrBuilder.append("\\encoding UTF8;\n");
    
    auxStrBuilder.append("CREATE SCHEMA grupo2v" + schema + ";\n");
    
    System.out.println("Reading create schema...");

    Files.readAllLines(Paths.get(CREATE_SCHEMA_SQL + ".sql"))
        .forEach(line -> auxStrBuilder
            .append(line.replaceAll("(Call[^e]|DateTime|EmisorReceptor)",
                "grupo2v" + schema + ".$1"))
            .append("\n"));
    ...
    List<String> userLines =
        Files.readAllLines(Paths.get(EMISOR_RECEPTOR_SQL + ".sql")).stream()
            .skip(EMISORRECEPTOR_BASE_COUNT - userAmount)
            .map(line -> line.replaceAll("(Call[^e]|DateTime|EmisorReceptor)",
                "grupo2v" + schema + ".$1"))
    ...
    Files.write(Paths.get(String.format(PATH + "data%d.sql", callAmount)),
        auxStrBuilder.toString().getBytes(),
    ...
    for (int callId = 1; callId <= callAmount; callId++) {
      if (callId % 10000 == 0) {
        System.out
            .println(String.format("Call progress: %.2f %%", (callId / (double) callAmount * 100)));
      }
  ...
  public static class RandomDateTime {
    ...
    public String toDateTime(int schema) {
      return new StringBuilder()
          ...
          .append("TIMESTAMP '" + timestamp + "'").append(", ").append(day).append(", ")
          .append(month)
  ...
  public static class Call {
    ...
    public String toCall(int schema) {
      ...
      for (int member : members) {
        strBuilder.append(
            "INSERT INTO grupo2v" + schema
                + ".Call (Id, TimeId, CallerId, MemberId, Duration) VALUES (");
