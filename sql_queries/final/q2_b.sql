...
CREATE OR REPLACE VIEW MemberPairs(MemberId1, MemberId2) AS (
    ...
    WHERE EmisorReceptor1.UserName <> EmisorReceptor2.UserName
      AND EmisorReceptor1.Operator <> EmisorReceptor2.Operator
...
-- Todos los pares de usuarios posibles.
UserPairsAll(UserName1, UserName2) AS (
...
UserDistancesAll(UserName1, UserName2, Distance) AS (
  ...
  UNION
    SELECT UserName1, UserName2, -1 FROM UserPairsAll WHERE NOT EXISTS (
        SELECT * FROM UserDistancesReflex 
        WHERE UserPairsAll.UserName1 = UserDistancesReflex.UserName1 AND UserPairsAll.UserName2 = UserDistancesReflex.UserName2
            OR UserPairsAll.UserName1 = UserDistancesReflex.UserName2 AND UserPairsAll.UserName2 = UserDistancesReflex.UserName1
    )   
)
